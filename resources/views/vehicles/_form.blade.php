...
<div class="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-6">
    ...
    <section class="bg-white border-2 border-gray-200 rounded-lg p-5">
        ...
        {{-- Sprzedaż przeniesiona na osobną kartę /vehicles/{id}/sell --}}
        @if($vehicle->exists)
            <div class="mb-6 p-3 bg-blue-50 border border-blue-200 rounded-lg text-center">
                <p class="text-sm text-gray-700 mb-2">💸 <strong>Sprzedaż auta</strong> jest na osobnej karcie</p>
                <a href="{{ route('vehicles.sale.edit', $vehicle) }}"
                   class="inline-block px-4 py-2 bg-blue-600 text-white text-sm font-bold rounded-lg hover:bg-blue-700">
                    {{ $vehicle->sale ? '✏️ Edytuj sprzedaż' : '💸 Sprzedaj to auto' }}
                </a>
            </div>
        @endif
