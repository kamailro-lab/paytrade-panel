...
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between flex-wrap gap-2">
            ...
            <div class="flex gap-2 flex-wrap">
                <form method="POST" action="{{ route('vehicles.dealerhub.sync', $vehicle) }}" class="inline">@csrf<button type="submit" class="px-4 py-2 bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700" title="Pobierz z DealerHub.ie">🔗 Pobierz z DealerHub</button></form>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 space-y-4">
            ...
            {{-- 📷 GALERIA ZDJĘĆ (DealerHub.ie) --}}
            @if(!empty($vehicle->photos) && is_array($vehicle->photos))
                <div class="bg-white shadow rounded-lg overflow-hidden">
                    <div class="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
                        <h3 class="font-bold text-gray-800">📷 Zdjęcia ({{ count($vehicle->photos) }})</h3>
                        <span class="text-xs text-gray-500">Źródło: DealerHub.ie</span>
                    </div>
                    <div class="p-4">
                        <div id="main-photo" class="mb-3 bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center" style="min-height:300px; max-height:500px;">
                            <img src="{{ $vehicle->photos[0] }}" alt="Główne zdjęcie {{ $vehicle->registration }}"
                                 class="w-full h-auto max-h-[500px] object-contain cursor-pointer"
                                 onclick="window.open(this.src, '_blank')">
                        </div>
                        <div class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2">
                            @foreach($vehicle->photos as $idx => $photoUrl)
                                <button type="button" onclick="document.getElementById('main-photo').querySelector('img').src='{{ $photoUrl }}'"
                                        class="aspect-square rounded overflow-hidden border-2 border-gray-200 hover:border-indigo-500 transition">
                                    <img src="{{ $photoUrl }}" alt="Zdjęcie {{ $idx + 1 }}" class="w-full h-full object-cover" loading="lazy">
                                </button>
                            @endforeach
                        </div>
                    </div>
